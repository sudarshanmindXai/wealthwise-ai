...
async def process_file_async(task_id: str, file_path: Path, filename: str):
    ...
    try:
        ...
        _tasks[task_id]["current_step"] = "Detecting document type..."
        ...
        if not parser:
            ...
            _tasks[task_id]["error"] = "Could not detect document type. Unsupported format."
            _tasks[task_id]["progress"] = 100
            return
        
        _tasks[task_id]["document_type"] = parser.DOCUMENT_TYPE.value
        _tasks[task_id]["current_step"] = f"Detected: {parser.DOCUMENT_TYPE.value}"
        _tasks[task_id]["progress"] = 20
        
        # Parse with streaming (simulated for now)
        _tasks[task_id]["status"] = "parsing"
        
        for progress_update in parser.parse_streaming(file_path):
            ...
            if progress_update.message:
                _tasks[task_id]["message"] = progress_update.message
        ...
        _tasks[task_id]["status"] = "complete" if result.success else "failed"
        _tasks[task_id]["progress"] = 100
        _tasks[task_id]["result"] = result.to_dict()
        _tasks[task_id]["current_step"] = "Complete" if result.success else "Failed"
        
        if result.errors:
            _tasks[task_id]["error"] = "; ".join(result.errors)
