...
async def get_transactions_for_review():
    ...
    with open("debug_review.txt", "w") as f:
        f.write(f"DEBUG /review/transactions: Found {len(all_tasks)} total tasks\n")
        
        for task in all_tasks:
            f.write(f"DEBUG Task: id={task.get('task_id')}, status={task.get('status')}, doc_type={task.get('document_type')}\n")
            
            # Check if task is complete and is a bank statement
            f.write(f"DEBUG Task Detail: id={task.get('task_id')}, filename={task.get('filename')}, error={task.get('error')}\n")
            
            if task.get("status") == "complete" and task.get("document_type") == "bank_statement":
                result = task.get("result", {})
                success = result.get("success", False)
                f.write(f"DEBUG Result success: {success}\n")
                
                if not success:
                    f.write(f"ERROR: Task {task.get('task_id')} failed: {result.get('errors')}\n")
                    continue
                ...
                f.write(f"DEBUG Found {len(transactions)} transactions in task {task.get('task_id')}\n")
                ...
                for idx, txn in enumerate(transactions):
                    try:
                        ...
                        f.write(f"Skipping malformed transaction in task {task.get('task_id')}: {e}\n")
                        continue
